...
relion_import_cmd = [dep.relion_import,' --i "', '*.eer" --ofile movies.star --angpix ' ,num2str(tomolist.pixelsize,'%f'), ' --odir ./ --do_movies'];
%disp(relion_import_cmd);
system(relion_import_cmd);

relion_run_motioncor_cmd = [dep.relionmc,' --i ','movies.star --j ',num2str(par.cpus_per_task),' --o ./MotionCorr', binfactor_str,bfactor_str,patch_str,eer_patch,gain_str,pixelsize_str,additional_str,oddeven_str];
